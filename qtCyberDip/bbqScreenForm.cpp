...
//----------------------------------------------------
void bbqScreenForm::connectTo(const QString &host)
{
	mHost = host;
...
//----------------------------------------------------
void bbqScreenForm::attemptConnection()
{
	mIsConnecting = true;
...
//----------------------------------------------------
void bbqScreenForm::setQuality(bool high)
{
	mHighQuality = high;
...
//----------------------------------------------------
void bbqScreenForm::setShowFps(bool show)
{
	mShowFps = show;
}
//----------------------------------------------------
void bbqScreenForm::processPendingDatagrams()
{
	...
	while (mTcpSocket.bytesAvailable() > 0)
	{
		...
		while (mGlobalBytesBuffer.size() > 10)
		{
			...
			//debugÓÃµÄÊä³ö
			//qDebug() << mGlobalBytesBuffer.left(headerSize).toHex().toStdString() << "  FS:" << (int)frameSize << "  AS:" << (int)audioFrameSize << std::endl;;
...
//----------------------------------------------------
void bbqScreenForm::onDecodeFinished(bool result, bool isAudio)
{
	if (!isAudio && result)
	{
...
//----------------------------------------------------
void bbqScreenForm::onSocketStateChanged()
{
	if (mStopped || !ui)
	{
	...
	{
		qDebug() << "Connected, loading first frame...";
	...
	{
		qDebug() << "Lost connection with host device. Reconnecting...";
...
//----------------------------------------------------
void bbqScreenForm::timerEvent(QTimerEvent *evt)
{
	if (mStopped)
		return;
...
bool bbqScreenForm::nativeEvent(const QByteArray& eventType, void* message, long* result)
{
...
//----------------------------------------------------
/*
void bbqScreenForm::keyReleaseEvent(QKeyEvent *evt)
{
	switch (evt->key())
	{
	case Qt::Key_Control:
...
//----------------------------------------------------
void bbqScreenForm::keyPressEvent(QKeyEvent *evt)
{
	switch (evt->key())
	{
	case Qt::Key_Control:
...
//----------------------------------------------------
void bbqScreenForm::mousePressEvent(QMouseEvent *evt)
{
...
//----------------------------------------------------
void bbqScreenForm::mouseReleaseEvent(QMouseEvent *evt)
{
	if (evt->button() == Qt::LeftButton || evt->button() == Qt::RightButton)
	{
		mIsMouseDown = false;
...
//----------------------------------------------------
void bbqScreenForm::mouseMoveEvent(QMouseEvent *evt)
{
	if (mIsMouseDown)
	{
		float posX = evt->x();
...
//----------------------------------------------------
*/
quint8 bbqScreenForm::bytesToUInt8(const QByteArray& bytes, int offset)
{
	return ((unsigned char) (bytes.at(offset)));
}
//----------------------------------------------------
quint16 bbqScreenForm::bytesToUInt16(const QByteArray& bytes, int offset)
{
	quint16 value = 0;
	for (int i = 0; i < 2; i++) {
...
//----------------------------------------------------
quint32 bbqScreenForm::bytesToUInt32(const QByteArray& bytes, int offset)
{
	quint32 value = 0;
	for (int i = 0; i < 4; i++) {
...
void bbqScreenForm::sendKeyboardInput(bool down, unsigned int keyCode)
{
...
//----------------------------------------------------
void bbqScreenForm::sendTouchInput(TouchEventType type, unsigned char finger, unsigned short x, unsigned short y)
{
	if (mTcpSocket.state() != QAbstractSocket::ConnectedState) return;
...
*/
QByteArray bbqScreenForm::numberToBytes(unsigned int value, int size)
{
	QByteArray output;
...
QPoint bbqScreenForm::getScreenSpacePoint(int x, int y)
{
	QPoint output;
